<script lang="ts">
	...
	import MultiImageOverlay from '$lib/components/generators/multi_image_overlay.svelte';
	...
	$: offser_note = get_offer_note(valid_packages);
</script>
...
<div>
	...
	<div class="possible_packages_container">
		...
		{#each possible_packages.slice(0, -1) as id}
			<p on:click={() => add_package(id)}>{id},</p>
		{/each}
		<p on:click={() => add_package(possible_packages.at(-1))}>
