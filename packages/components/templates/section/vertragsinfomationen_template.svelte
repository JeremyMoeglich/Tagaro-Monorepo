...
<div>
	...
	<p>
		...
		Netflix-Dienst wie oben näher beschrieben. Für Netflix-Accounts, die mit Sky Entertainment Plus
		verbunden sind, gilt für die Dauer des Sky Entertainment Plus-Abonnements folgendes: Die
		Geschäftsbedingungen von Sky regeln das Abonnementverhältnis des Kunden mit Sky sowie das Sky
		Entertainment Plus-Abonnement selbst, während die Nutzungsbedingungen von Netflix die Benutzung
		des Netflix-Dienstes durch den Kunden regeln. Alle Ansprüche des Kunden im Hinblick auf das
		Entertainment Plus-Abonnement müssen gegenüber Sky geltend gemacht werden. Sky erklärt hiermit
		im Namen von Netflix, dass die Netflix-Nutzungsbedingungen in Bezug auf die Laufzeit, die
		Rechnungsstellung, den Preis, ausdrückliche Zusicherungen/Gewährleistungsansprüche/Garantien
		(soweit vorhanden), Probezeiträume und Kündigungen nicht anwendbar sind. Und Netflix wird in
		Bezug auf den jeweiligen Netflix-Account keine Entgelte in Rechnung stellen.
	</p>
</div>
<div>
	...
	<p>
		...
		Netflix Standard oder Premium vorgenommen werden. Netflix Standard beinhaltet aktuell HD und die
		gleichzeitige Nutzung von Netflix auf bis zu zwei Geräten. Netflix Premium beinhaltet aktuell
		UHD und die gleichzeitige Nutzung von Netflix auf bis zu vier Geräten. Das Sky Entertainment
		Plus Upgrade kann vom Kunden jederzeit sowohl mittels Erklärung gegenüber Sky als auch gegenüber
		Netflix beendet werden. Im letzteren Fall (Erklärung der Kündigung gegenüber Netflix) handelt
		Netflix als Empfangsbote für Sky. Das Entertainment Plus Paket enthält ab dem Wirksamwerden
		dieser Kündigung wiederum nur noch Netflix Basis oder Netflix Standard.
	</p>
</div>
<div>
	<h3>DAZN über Sky:</h3>
	<p>
		...
		beiden Sender aus lizenzrechtlichen Gründen derzeit nicht verfügbar. Kunden, die Sky über dieses
		Netz empfangen, können ebenfalls wie Kunden anderer Netze, die DAZN Inhalte über Sky Q in der
		DAZN App empfangen. Zum Empfang über die DAZN App ist eine dauerhafte Internetverbindung nötig.
		Empfohlene Übertragungsrate von mindestens 8 Mbit/s für HD-Qualität. Durch eine parallele
		Nutzung von Inhalten erhöht sich die benötigte Mindestbandbreite. Erst mit Aktivierung des
		DAZN-Abonnements beginnt dessen zahlungspflichtige Laufzeit. Sky übersendet per E-Mail einen
		DAZN-Aktivierungslink. Über diesen kann das DAZN-Abonnement direkt bei DAZN aktiviert werden.
		Die Aktivierung ist bis 7 Tage ab Versand des Aktivierungslinks möglich. Falls das
		DAZN-Abonnement nicht innerhalb dieser Frist aktiviert wird, wird die Buchung automatisch nach 7
		Tagen storniert. Vertragspartner ist die DAZN Limited, Hanover House, Plane Tree Crescent,
		Feltham, Vereinigtes Königreich, TW137BZ. Voraussetzung für die Nutzung der linearen Kanäle im
...
<div>
	...
	<p>
		...
		gemäß EU-Portabilitätsverordnung auch während eines vorübergehenden Aufenthalts in einem anderen
		EU-Mitgliedstaat als Österreich oder Deutschland nutzen. Mit der Download-Funktion können bis zu
		25 Titel (max. 5 Spielfilme) gleichzeitig und jeder Titel kann nur zweimal im Kalenderjahr
		heruntergeladen werden. Die heruntergeladenen Titel bleiben jeweils 30 Tage verfügbar. Nach
		Beginn der Wiedergabe stehen die Titel 48 Stunden lang zur Verfügung und werden anschließend
		gesperrt.
	</p>
</div>
<div>
	<h3>Sky Go Plus:</h3>
	<p>
		Mit Sky Go Plus können einzelne Inhalte des Sky Abonnements auf 3 mobilen Endgeräten mit der Sky
		Go App gestreamt werden (ausgeschlossen ist die Verbindung über einen VPN-Zugang).
	</p>
</div>
<div>
	<h3>UHD:</h3>
	<p>
		...
		Programmpaketen und der Verfügbarkeit beim jeweiligen Netzbetreiber. Weitere Informationen unter
		www.sky.de/senderempfang. Vereinzelt sind aus lizenzrechtlichen Gründen bestimmte Inhalte nicht
		in UHD verfügbar. Über die Sky Q IPTV Box und Sky Q-Mini sind UHD-Inhalte nicht verfügbar.
	</p>
</div>
<div>
	...
	<p>
		...
		sowie eine stabile Internetverbindung (ausgeschlossen ist die Verbindung über einen VPN-Zugang).
		Empfohlene Bandbreite 6 Mbit/s für HD-Qualität. Für die Internetverbindung anfallende Kosten
		sind nicht Bestandteil des Abonnements. Das Streaming des Sky Programmes ist mit der Sky Q-Mini,
		der Sky Q IPTV Box, der Sky Q App (auf Apple TV ab Generation 4, auf Smart TV von LG Electronics
		mit Betriebssystem Web OS 2.0 oder höher oder Samsung Smart TV ab Modelljahr 2015 mit Tizen
		Betriebssystem oder einer Playstation 4 Konsole) und mit der Sky Go App auf mobilen Endgeräten
		(Betriebssysteme: Android, iOS, Windows; Hersteller: Mac, Huawei) möglich. Über die Sky Q IPTV
		Box und Sky Q-Mini sind UHD-Inhalte nicht verfügbar. Pro Sky Q Abonnement über SAT oder Kabel
		sind maximal 2 Sky Q Receiver (je € 49 durch Sky) zzgl. maximal 2 Sky Q-Mini (je € 29 durch Sky)
		oder bei einem Sky Q Abonnement über das Internet maximal 3 Sky Q IPTV Boxen (je € 29 durch Sky)
		buchbar. Eine Buchung von Multiscreen ohne zusätzliches Gerät ist ebenso möglich.
	</p>
</div>
<div>
	<h3>Apps:</h3>
...
<div>
	...
	<p>
		...
		Abonnement verlängert sich nach Ablauf der Mindestlaufzeit um jeweils einen weiteren Monat, wenn
		es nicht mit einer Frist von 1 Monat auf den Ablauf der Laufzeit gekündigt wird. Danach beträgt
		die Abonnementgebühr € 6 mtl. Vertragspartner des HD+ Abonnements ist die HD PLUS GmbH. Die HD
		PLUS GmbH ermächtigt Sky, die Abonnementgebühren einzuziehen.
	</p>
</div>
<div>
...
<div>
	...
	<p>
		Die Sky Q IPTV Box wird leihweise für die Dauer des Sky Abonnements zur Verfügung gestellt. Mit
		der Sky Q IPTV Box können einzelne Inhalte des Sky Abonnements über das Internet auf einem
...
<div>
	...
	<p>
		...
		meisten Fällen nicht übernommen werden. Sie erhalten kostenlos neue Geräte zugesendet und können
		bestehende Geräte einfach nach Ablauf Ihres Vertrags an Sky retournieren. Der Abobeginn ist bei
		Sky nicht nach hinten verlegbar. Der Vertrag startet innerhalb von 7 Tagen nach Erhalt der Ware.
	</p>
</div>
<div>
...
<style>
	...
	h3 {
		background: linear-gradient(to right, rgb(0, 4, 245) 0%, rgb(0, 132, 255) 100%);
