<script lang="ts">
	...
	import {
		...
		set_preferences,
		preferences_store
	} from 'frontend/preferences';
	import type { preferences_obj } from 'frontend/preferences';
	import Datenschutz from 'components/complete/service/datenschutz.svelte';
...
{#if is_banner_shown($preferences_store, banner_enabled)}
	...
	<div class="outer">
		...
		<div class="buttons">
			<div on:click={save_current_preferences} on:keypress={enter_filter(save_current_preferences)}>
				<Button text="Einstellungen Speichern" reversed={true} />
			</div>
			<div
				on:click={save_accept_all_preferences}
				on:keypress={enter_filter(save_accept_all_preferences)}
			>
