	...
	onSnapshot(
		...
		(snapshot) => {
			ip_logs = sortBy(
				...
				(item: log_type) => item.createdAt.seconds
			);
		}
	);
...
<div class="flex w-full h-full items-center justify-center">
	<div class="flex bg-slate-700 pr-4 rounded-lg shadow-md gap-4 items-center text-slate-300">
		...
		<div class="flex flex-col gap-4">
			{#each log_watchlist as watchlist_item}
				<div class="flex items-center gap-4 bg-slate-800 p-2 rounded">
					<div class="left">
						<div>
							{watchlist_item.ip}
						</div>
